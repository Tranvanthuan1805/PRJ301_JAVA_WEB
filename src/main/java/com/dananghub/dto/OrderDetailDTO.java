...
public class OrderDetailDTO {
    ...
    public OrderDetailDTO(Order order, Tour tour, String username) {
        ...
        // Get quantity from first booking
        if (order.getBookings() != null && !order.getBookings().isEmpty()) {
            this.numberOfPeople = order.getBookings().get(0).getQuantity();
        } else {
            this.numberOfPeople = 1; // default
        }
        ...
        if (tour != null) {
            ...
            this.tourLocation = tour.getDestination();
        }
    }
    ...
    public String getStatusDisplay() { return getStatusEnum().getDisplayName(); }
    ...
    public int getQuantity() { return numberOfPeople; }
