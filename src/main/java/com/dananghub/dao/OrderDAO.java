...
public class OrderDAO {
    ...
    public boolean cancelOrder(int orderId, String reason) {
        ...
        try {
            ...
            if (order != null && order.canCancel()) {
                // Return tour capacity before cancelling
                ...
                for (Booking booking : bookings) {
                    Tour tour = em.find(Tour.class, booking.getTour().getId());
                    if (tour != null) {
                        // Decrease currentCapacity (return seats)
                        tour.setCurrentCapacity(tour.getCurrentCapacity() - booking.getQuantity());
                        em.merge(tour);
                    }
                }
    ...
    public List<OrderDetailDTO> getOrderDetailsByCustomer(int customerId) {
        ...
        try {
            ...
            for (Order order : orders) {
                // Get booking quantity directly from database
                Integer quantity = (Integer) em.createQuery(
                    "SELECT b.quantity FROM Booking b WHERE b.order.orderId = :oid")
                    .setParameter("oid", order.getOrderId())
                    .setMaxResults(1)
                    .getSingleResult();
                
                // Get tour from booking
                Tour tour = (Tour) em.createQuery(
                    "SELECT b.tour FROM Booking b WHERE b.order.orderId = :oid")
                    .setParameter("oid", order.getOrderId())
                    .setMaxResults(1)
                    .getSingleResult();
                ...
                OrderDetailDTO dto = new OrderDetailDTO(order, tour, username);
                if (quantity != null) {
                    dto.setNumberOfPeople(quantity);
                }
                result.add(dto);
            }
        } catch (Exception e) {
