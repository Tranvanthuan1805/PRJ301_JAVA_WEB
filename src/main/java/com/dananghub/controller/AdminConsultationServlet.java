...
public class AdminConsultationServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            e.printStackTrace();
            getServletContext().log("AdminConsultationServlet ERROR: " + e.getMessage(), e);
            // Set empty defaults so page still renders
            req.setAttribute("consultations", new ArrayList<>());
            req.setAttribute("totalNew", 0L);
            req.setAttribute("totalContacted", 0L);
            req.setAttribute("totalDone", 0L);
            req.setAttribute("totalAll", 0L);
            req.setAttribute("currentStatus", "");
