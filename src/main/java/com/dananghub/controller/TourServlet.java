...
public class TourServlet extends HttpServlet {
    ...
    private void listTours(HttpServletRequest request, HttpServletResponse response)
        ...
        // Category filter - REMOVED (database không có category)
        ...
        if (sortBy != null && !sortBy.isEmpty()) {
            switch (sortBy) {
                case "name_asc" -> allTours.sort((t1, t2) -> t1.getName().compareToIgnoreCase(t2.getName()));
                case "name_desc" -> allTours.sort((t1, t2) -> t2.getName().compareToIgnoreCase(t1.getName()));
