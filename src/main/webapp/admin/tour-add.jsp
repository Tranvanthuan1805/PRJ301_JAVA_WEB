...
<head>
    ...
    <title>Thêm Tour Mới | eztravel</title>
...
<main class="main">
    <div class="page-header">
        <h1><i class="fas fa-plus-circle"></i> Thêm Tour Mới</h1>
        <p>Tạo tour du lịch mới cho năm 2026</p>
    </div>
    ...
    <div class="form-container">
        <form method="post" action="${pageContext.request.contextPath}/admin/tours/add">
            <div class="form-group">
                <label><i class="fas fa-tag"></i> Tên Tour <span class="required">*</span></label>
                <input type="text" name="name" required placeholder="VD: Bà Nà Hills - Cầu Vàng Tháng 3" value="${param.name}">
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="destination" required>
                        ...
                        <option value="Bà Nà Hills" ${param.destination == 'Bà Nà Hills' ? 'selected' : ''}>Bà Nà Hills</option>
                        <option value="Ngũ Hành Sơn" ${param.destination == 'Ngũ Hành Sơn' ? 'selected' : ''}>Ngũ Hành Sơn</option>
                        <option value="Cù Lao Chàm" ${param.destination == 'Cù Lao Chàm' ? 'selected' : ''}>Cù Lao Chàm</option>
                        <option value="Sơn Trà" ${param.destination == 'Sơn Trà' ? 'selected' : ''}>Sơn Trà</option>
                        <option value="Huế" ${param.destination == 'Huế' ? 'selected' : ''}>Huế</option>
                        <option value="Núi Thần Tài" ${param.destination == 'Núi Thần Tài' ? 'selected' : ''}>Núi Thần Tài</option>
                    </select>
                </div>
                
                <div class="form-group">
                    <label><i class="fas fa-dollar-sign"></i> Giá (VNĐ) <span class="required">*</span></label>
                    <input type="number" name="price" required min="0" step="1000" placeholder="VD: 1200000" value="${param.price}">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" name="startDate" required min="2026-01-01" max="2026-12-31" value="${param.startDate}">
                </div>
                
                <div class="form-group">
                    <label><i class="fas fa-calendar-check"></i> Ngày Kết Thúc <span class="required">*</span></label>
                    <input type="date" name="endDate" required min="2026-01-01" max="2026-12-31" value="${param.endDate}">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" name="maxCapacity" required min="1" placeholder="VD: 40" value="${param.maxCapacity}">
                </div>
                
                <div class="form-group">
                    <label><i class="fas fa-user-check"></i> Số Người Đã Đặt</label>
                    <input type="number" name="currentCapacity" min="0" placeholder="VD: 0" value="${param.currentCapacity != null ? param.currentCapacity : '0'}">
                </div>
            </div>
            
            <div class="form-group">
                <label><i class="fas fa-align-left"></i> Mô Tả</label>
                <textarea name="description" placeholder="Mô tả chi tiết về tour...">${param.description}</textarea>
            </div>
