...
<head>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    /* Buttons */
    .btn{padding:8px 16px;border:none;border-radius:8px;cursor:pointer;font-size:.82rem;font-weight:600;transition:.3s;font-family:inherit;display:inline-flex;align-items:center;gap:6px}
    ...
    .btn-primary{background:rgba(59,130,246,.2);color:#60A5FA;border:1px solid rgba(59,130,246,.3)}
...
<aside class="sidebar">
    ...
    <nav>
        ...
        <a href="${pageContext.request.contextPath}/admin/orders" class="active"><i class="fas fa-shopping-bag"></i> Đơn Hàng</a>
...
<main class="main">
    <header class="page-header">
        <div>
            ...
            <p>Xem và xử lý tất cả đơn hàng từ khách hàng</p>
        </div>
    ...
    <section class="stats">
        <div class="stat">
            ...
            <div class="value" style="color:#FBBF24">${pendingCount}</div>
        </div>
        <div class="stat">
            <div class="icon icon-blue"><i class="fas fa-check-circle"></i></div>
            <div class="label">Đã Xác Nhận</div>
            <div class="value" style="color:#60A5FA">${confirmedCount}</div>
        </div>
        <div class="stat">
            <div class="icon icon-green"><i class="fas fa-check-double"></i></div>
            <div class="label">Hoàn Thành</div>
            <div class="value" style="color:#34D399">${completedCount}</div>
        </div>
        <div class="stat">
            <div class="icon icon-red"><i class="fas fa-times-circle"></i></div>
            <div class="label">Đã Hủy</div>
            <div class="value" style="color:#F87171">${cancelledCount}</div>
        </div>
    </section>
                ...
                <table>
                    ...
                    <tbody>
                        <c:forEach items="${orders}" var="order">
                            <tr>
                                <td><strong style="color:#60A5FA">#${order.orderId}</strong></td>
                                <td>${order.customer.fullName}</td>
                                <td><fmt:formatDate value="${order.orderDate}" pattern="dd/MM/yyyy HH:mm"/></td>
                                <td style="color:#34D399;font-weight:700"><fmt:formatNumber value="${order.totalAmount}" type="number" groupingUsed="true"/>đ</td>
                                <td><span class="badge badge-${order.statusBadgeClass}">${order.statusDisplayName}</span></td>
                                <td>${order.paymentStatus}</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${order.orderStatus == 'Pending'}">
                                            <form action="${pageContext.request.contextPath}/admin/orders" method="post" style="display:inline;margin-right:6px;">
                                                <input type="hidden" name="action" value="confirm">
                                                <input type="hidden" name="orderId" value="${order.orderId}">
                                                <button type="submit" class="btn btn-success" title="Xác nhận đơn hàng"><i class="fas fa-check"></i> Chấp nhận</button>
                                            </form>
                                            <form action="${pageContext.request.contextPath}/admin/orders" method="post" style="display:inline;margin-right:6px;" onsubmit="return confirm('Bạn có chắc muốn hủy đơn hàng #${order.orderId}?')">
                                                <input type="hidden" name="action" value="cancel">
                                                <input type="hidden" name="orderId" value="${order.orderId}">
                                                <button type="submit" class="btn btn-warning" title="Hủy đơn hàng"><i class="fas fa-ban"></i> Hủy</button>
                                            </form>
                                            <form action="${pageContext.request.contextPath}/admin/orders" method="post" style="display:inline;" onsubmit="return confirm('Bạn có chắc muốn XÓA VĨNH VIỄN đơn hàng #${order.orderId}?')">
                                                <input type="hidden" name="action" value="delete">
                                                <input type="hidden" name="orderId" value="${order.orderId}">
                                                <button type="submit" class="btn btn-danger" title="Xóa đơn hàng"><i class="fas fa-trash"></i></button>
                                            </form>
                                        </c:when>
                                        <c:when test="${order.orderStatus == 'Confirmed'}">
                                            <form action="${pageContext.request.contextPath}/admin/orders" method="post" style="display:inline;margin-right:6px;">
                                                ...
                                                <input type="hidden" name="orderId" value="${order.orderId}">
                                                <button type="submit" class="btn btn-primary" title="Hoàn thành đơn hàng"><i class="fas fa-check-double"></i> Hoàn thành</button>
                                            </form>
                                            <form action="${pageContext.request.contextPath}/admin/orders" method="post" style="display:inline;margin-right:6px;" onsubmit="return confirm('Bạn có chắc muốn hủy đơn hàng #${order.orderId}?')">
                                                <input type="hidden" name="action" value="cancel">
                                                <input type="hidden" name="orderId" value="${order.orderId}">
                                        ...
                                        <c:otherwise>
                                            <form action="${pageContext.request.contextPath}/admin/orders" method="post" style="display:inline;" onsubmit="return confirm('Bạn có chắc muốn XÓA VĨNH VIỄN đơn hàng #${order.orderId}?')">
                                                <input type="hidden" name="action" value="delete">
                                                <input type="hidden" name="orderId" value="${order.orderId}">
                                                <button type="submit" class="btn btn-danger" title="Xóa đơn hàng"><i class="fas fa-trash"></i> Xóa</button>
                                            </form>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
...
if (urlParams.get('msg') === 'success') {
    ...
    window.history.replaceState({}, '', '${pageContext.request.contextPath}/admin/orders');
} else if (urlParams.get('error')) {
    alert('✗ Có lỗi xảy ra: ' + urlParams.get('error'));
    window.history.replaceState({}, '', '${pageContext.request.contextPath}/admin/orders');
}
</script>
